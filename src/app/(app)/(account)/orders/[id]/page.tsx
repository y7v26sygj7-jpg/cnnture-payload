    ...
    <div className="">
      <div className="flex gap-8 justify-between items-center mb-6">
        ...
        <h1 className="text-sm uppercase font-mono px-2 bg-primary/10 rounded tracking-[0.07em]">
          <span className="">{t('orders.order_number')}{order.id}</span>
        </h1>
      </div>
      ...
      <div className="bg-card border rounded-lg px-6 py-4 flex flex-col gap-12">
        <div className="flex flex-col gap-6 lg:flex-row lg:justify-between">
          <div className="">
            <p className="font-mono uppercase text-primary/50 mb-1 text-sm">{t('orders.order_date')}</p>
            <p className="text-lg">
          ...
          {order.status && (
            <div className="grow max-w-1/3">
              <p className="font-mono uppercase text-primary/50 mb-1 text-sm">{t('orders.status')}</p>
        ...
        {order.items && (
          <div>
            <h2 className="font-mono text-primary/50 mb-4 uppercase text-sm">{t('orders.items')}</h2>
            <ul className="flex flex-col gap-6">
              {order.items?.map((item, index) => {
                ...
                if (!item.product || typeof item.product !== 'object') {
                  return <div key={index}>{t('orders.item_unavailable')}</div>
                }
        ...
        {order.shippingAddress && (
          <div>
            <h2 className="font-mono text-primary/50 mb-4 uppercase text-sm">{t('orders.shipping_address')}</h2>
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    description: `${t('orders.order_number')}${id}`,
    openGraph: mergeOpenGraph({
      title: `${t('orders.order_number')}${id}`,
      url: `/orders/${id}`,
    }),
    title: `${t('orders.order_number')}${id}`,
  }
